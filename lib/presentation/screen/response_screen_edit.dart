...
class ResponseScreenEdit extends StatefulWidget {
  ...
  @override
  _ResponseScreenEditState createState() => _ResponseScreenEditState(
...
class _ResponseScreenEditState extends BaseState<ResponseScreenEdit> {
  final String plantTypeName;
  final int exchangeRequestID;
  final String username;
  final int responseID;

  _ResponseScreenEditState({
    @required this.plantTypeName,
    @required this.exchangeRequestID,
    @required this.username,
    @required this.responseID,
  ...
  Widget build(BuildContext context) {
    return BlocListener<ResponseBloc, ResponseState>(
      listener: (listenerContext, state) {
        if (state is ResponseEditFail) {
          ScaffoldMessenger.of(listenerContext)
            ..hideCurrentSnackBar()
            ..showSnackBar(
              SnackBar(
                backgroundColor: Colors.red,
                content: Text(translator.text("edit_response_fail")),
              ),
            );
          Navigator.pop(context);
        }

        if (state is ResponseEditSuccess) {
          ScaffoldMessenger.of(listenerContext)
            ..hideCurrentSnackBar()
            ..showSnackBar(
              SnackBar(
                backgroundColor: Colors.green,
                content: Text(translator.text("edit_response_success")),
              ),
            );
          Navigator.pop(context);
        }
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        BlocBuilder<ResponseBloc, ResponseState>(
                          builder: (context, state) {
                            if (state is RequestWeightLoaded) {
                              exchangeYield = state.request.weight;
